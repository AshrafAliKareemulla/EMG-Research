...
- **Scientific questions tracking (Columns 21–35):** one column per scientific-question. Each holds a structured Y/N/Partial fill per sub-question, so you can filter the sheet by "papers that address LOSO", "papers that test SSL" etc.
...
## Group 3 — Scientific questions tracking columns (Cols 20–35)
...
### Column 35. Paper Extraction
Clearly mention if the mineru paper extraction is clear and there is no ambiguity in the tables, math equations and other text parts. Flag any irregularity or any extraction issues while going through its contents. In case of any issues, always fall back to step 2 that is to read and extract the pdf paper on your own and properly highlight this when you do it yourself in this column with other details.
...
**Rule 4 — Two-pass extraction.** For doing deep literature review, for each paper, read the paper twice. Disagreements flag fields where the paper is genuinely ambiguous — exactly where a human should review.
